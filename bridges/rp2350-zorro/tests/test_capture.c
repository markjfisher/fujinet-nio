...
static void capture(epio_t *e, uint32_t word) {
    epio_step_cycles(e, 1); state(e, 0, 0); /* WAIT LOW */
    epio_step_cycles(e, 1); state(e, 0, 0); /* IN */
    /* Change the bus after IN: PUSH must contain the already sampled data. */
    epio_drive_gpios_ext(e, 0x3e, 0);
    epio_step_cycles(e, 1); state(e, 1, 0); /* PUSH */
    CHECK(epio_pop_rx_fifo(e, 0, 0) == word);
    epio_step_cycles(e, 1); state(e, 0, 1); /* IRQ */
    epio_clear_block_irq(e, 0, 0);
    state(e, 0, 0);
    epio_step_cycles(e, 32); state(e, 0, 0); /* held low */
}
int main(void) {
    ...
    epio_step_cycles(e, 32); state(e, 0, 0);
    epio_drive_gpios_ext(e, 0x3e, 0x28);
    capture(e, 0x0000000a);
    epio_drive_gpios_ext(e, 0x3e, 0x16); /* release with next value 0x5 */
    epio_step_cycles(e, 1); state(e, 0, 0); /* WAIT HIGH / wrap */
    epio_step_cycles(e, 16); state(e, 0, 0);
