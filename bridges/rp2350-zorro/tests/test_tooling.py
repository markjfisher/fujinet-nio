...
class Tooling(unittest.TestCase):
    def test_bootstrap_idempotence_stale_and_dirty(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            git("tag", "fixture-v1")
            pin = {"url": str(origin), "version": "fixture-v1", "revision": git("rev-parse", "HEAD")}
            ...
            with self.assertRaisesRegex(ValueError, "dirty source.*Preserve local work"):
                bootstrap.setup(checkout, pin)
    ...
    def test_registered_submodules_and_cli_overrides(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            sdk = root / "sdk-origin"
            repository(sdk)
            git(sdk, "-c", "protocol.file.allow=always", "submodule", "add", str(sub), "lib/fixture")
            git(sdk, "commit", "-qam", "register submodule")
            pin = {"revision": git(sdk, "rev-parse", "HEAD"), "submodules": ["lib/fixture"]}
            ...
            shutil.copyfile(ROOT / "scripts/bootstrap.py", fixture / "scripts/bootstrap.py")
            pins = {}
            ...
            def cli(mode, sdk_path):
                return subprocess.run([sys.executable, str(fixture / "scripts/bootstrap.py"),
                                       "--mode", mode, "--check"], capture_output=True, text=True,
    ...
    def test_guard_rejects_sources_and_build_rules(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            for name, content in [("capture.pio", ".program capture"),
                ...
                result = subprocess.run(["python3", str(ROOT / "scripts/check_pio_policy.py"), str(root)], capture_output=True)
                self.assertNotEqual(result.returncode, 0)
                path.unlink()
            (root / ".deps").mkdir()
