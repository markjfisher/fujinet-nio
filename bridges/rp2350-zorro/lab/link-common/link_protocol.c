...
void link_test_fill_payload(uint8_t *data, size_t length,
    ...
    for (i = 0; i < length; ++i) {
        switch (pattern) {
        case LINK_PATTERN_ZERO: data[i] = 0; break;
        case LINK_PATTERN_FF: data[i] = 0xff; break;
        case LINK_PATTERN_INCREMENT: data[i] = (uint8_t)(seed + i); break;
        case LINK_PATTERN_ALTERNATING: data[i] = (i & 1u) ? 0x55 : 0xaa; break;
...
enum link_test_status link_test_validate_frame(const struct link_test_frame *frame) {
    if (frame->magic != LINK_TEST_MAGIC) return LINK_STATUS_BAD_MAGIC;
    if (frame->version != LINK_TEST_VERSION) return LINK_STATUS_BAD_VERSION;
    if (frame->payload_length > LINK_TEST_MAX_PAYLOAD) return LINK_STATUS_BAD_LENGTH;
...
const char *link_test_status_name(enum link_test_status status) {
    static const char *const names[] = {"ok", "bad_magic", "bad_version", "bad_length", "bad_checksum", "unsupported"};
