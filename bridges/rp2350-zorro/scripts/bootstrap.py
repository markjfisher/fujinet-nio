...
def setup(path, pin, check_only=False):
    if not path.exists():
        if check_only:
            raise ValueError(
                f"Missing dependency {path}; run python3 scripts/bootstrap.py --mode host or --mode firmware."
            )
        ...
        if git(path, "rev-parse", "HEAD") != pin["revision"]:
            raise ValueError(
                f"Tag revision mismatch at {path}; remove checkout after inspection."
            )
...
def main():
    ...
    parser.add_argument(
        "--mode", choices=["host", "firmware", "stimulus"], required=True
    )
    parser.add_argument(
    ...
    names = (
        ["apio", "epio"] if args.mode == "host" else ["apio", "pico-sdk", "picotool"]
    )
    for name in names:
