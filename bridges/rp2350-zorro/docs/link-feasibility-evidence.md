...
This ledger preserves the reviewed physical outcomes of L0–L10 and the
...
| L6 | `build/link-feasibility/L6/20260922T182838Z-d2d42ff6` | A 32-byte partial request produced the expected ESP error response; force-reloading the RP2350 SRAM image before it was consumed, then retiring the stale state, allowed an exact fresh sequence-2 64-byte exchange. |
| L7 | `build/link-feasibility/L7/20260922T184950Z-58413b02` | A feasibility-controlled ESP restart at a completed-slot boundary was detected by the RP2350 through withdrawal of both flow-control outputs; after restart, an exact fresh sequence-2 64-byte exchange passed. |
...
| L9 | `build/link-feasibility/L9/20260922T220158Z-dff6d19f` | A 100-transfer, 64-byte recovery soak injected nine 32-byte partial slots. Each generated an explicit `bad_checksum` error frame, was drained, and was followed by exact valid traffic; no stale error became a later accepted echo. |
| L10 | `build/link-feasibility/L10/20260922T215630Z-6d6bc234` | All 54 polling/DMA profile batches passed. At actual 6.818 MHz and 240 bytes, polling was 105,655 B/s and DMA 105,646 B/s: indistinguishable at this precision. Each direction’s slot transfer averaged 357 us; the echo preparation wait averaged 907 us and READY re-arm 480 us. |
...
| Receiver unavailability is observable in this fixture. | L3's declared simulated pauses held the master until the ESP endpoint became ready and then recovered. Actual queue depth and sustained-pressure behavior remain open; throughput is characterized only for the current L8 fixture. |
...
| CPU-driven RP2350 SPI FIFO servicing is not the present throughput bottleneck. | L10’s 54 exact-frame batches show polling and SPI-DREQ DMA within measurement noise at every tested rate and payload size. For 240 bytes at actual 6.818 MHz, the two slot transfers consume 714 us of the ~2,272 us exchange; ESP echo preparation waits 907 us, READY re-arm 480 us, and the remaining ~171 us is local frame/loop work outside the phase timers. DMA remains a valid future implementation option for CPU-concurrency or descriptor reasons, but this fixture does not justify it as a throughput fix. |
| A partial transfer’s error outcome is a generation that must be observed before subsequent work starts. | L9’s first run exposed a real lab-runner race: submitting the next request before the ESP advertised the partial-slot error caused that request to receive the stale error response. The corrected run waits for the explicit error generation, drains it, and then proceeds. Its 100 accepted transfers plus nine injected partial/error pairs show that an eventual ABI needs a defined error-completion/clear boundary; a simple cached level cannot safely imply that the previous exchange is retired. |
| Reset/reload must discard or explicitly retire incomplete peer state before new ownership begins. | L6 force-reloaded the RP2350 SRAM image after an incomplete 32-byte slot. The old ESP error frame was retired, then a fresh sequence-2 exchange passed exactly. A later ABI needs a reset generation or equivalent clear/reinitialization rule; host RAM reload is not itself a production reset definition. |
| Peer reset must withdraw flow control and leave previous ownership ambiguous until a fresh generation is established. | L7's controlled ESP restart withdrew both flow-control outputs, was detected by the RP2350, and was followed by an exact fresh sequence-2 exchange. The ABI needs defined reset detection and reinitialization semantics; this does not prove raw power loss, USB disconnect, or arbitrary clock-edge reset behavior. |
