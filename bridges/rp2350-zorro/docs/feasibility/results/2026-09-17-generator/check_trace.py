...
for n, f in enumerate(fall):
    ...
    before = f
    while before > 0 and data[before - 1] & 15 == n:
        before -= 1
    after = r
    while after < len(data) and data[after] & 15 == n:
        after += 1
    if n > 0:
        assert 98 <= f - before <= 102, (n, "setup", f - before)
    if n < 15:
        assert 98 <= after - r <= 102, (n, "hold", after - r)
    if n > 0:
        assert 298 <= f - fall[n - 1] <= 302, (n, "period", f - fall[n - 1])
    rows.append(
        dict(
            value=n,
            fall_sample=f,
