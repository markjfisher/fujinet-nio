...
def generate_esp32_block(all_cpp):
    ...
    Notes:
      - PlatformIO/ESP-IDF builds define transport selection via *compiler* flags (e.g. -DFN_BUILD_ATARI_SIO),
        not as CMake cache variables. Therefore, we do NOT feature-gate sources here via CMake expressions.
        Instead, we compile all sources and rely on compile-time gating in code + the linker's object selection.
    """
    filtered = []
    for path in all_cpp:
        if path == "src/app/main_posix.cpp":
