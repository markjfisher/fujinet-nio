...
def cmd_list(args) -> int:
    ...
    for e in lr.entries:
        ...
        print(f"{kind} {e.size_bytes:10d}  {fp.fmt_utc(e.mtime_unix)}  {e.name}")
...
def main() -> None:
    ...
    ps = sub.add_parser("stat", help="Stat a file/dir via FileDevice")
    ps.add_argument("fs")
    ps.add_argument("path")
    ps.set_defaults(fn=cmd_stat)

    pr = sub.add_parser("read", help="Read a single chunk")
    ...
    pra = sub.add_parser("read-all", help="Read whole file in chunks")
    ...
    pw = sub.add_parser("write", help="Write a local file to the remote path in chunks")
