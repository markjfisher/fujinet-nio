...
def _parse_clock_time_resp(payload: bytes) -> int:
    ...
    if ver != CLOCKPROTO_VERSION:
        raise ValueError(
            f"clock version mismatch: got {ver}, want {CLOCKPROTO_VERSION}"
        )
    # flags = payload[1]
...
def register_subcommands(subparsers) -> None:
    ...
    pcg = csub.add_parser(
        "get",
        help="Get device time as Unix timestamp",
    ...
    pcs = csub.add_parser(
        "set",
        help="Set device time from this machine (UTC now)",
    ...
    pcgf = csub.add_parser(
        "get-format",
        help="Get time in a specific format using device's current timezone",
...
Formats:
  ...
  sos      - 16 bytes: Apple III SOS format (hex)""",
    ...
    pcgf.add_argument(
        "format", help="Format name: simple, prodos, apetime, iso-tz, iso-utc, sos"
    )
    pcgf.set_defaults(fn=cmd_clock_get_format)

    # get-tz: Get time with a specific timezone
    pcgt = csub.add_parser(
        "get-tz",
        help="Get time in a specific timezone and format",
...
Note: 'iso-utc' always shows +0000 offset but the time value is converted to the specified timezone.
    ...
    pcgt.add_argument(
        "timezone",
        help="POSIX timezone string (e.g., 'UTC', 'EST5EDT,M3.2.0,M11.1.0', 'UTC+3')",
    )
    pcgt.add_argument(
        "format", help="Format name: simple, prodos, apetime, iso-tz, iso-utc, sos"
    )
    ...
    pcgtz = csub.add_parser(
        "get-timezone",
        help="Get the device's current timezone setting",
    ...
    pcst = csub.add_parser(
        "set-timezone",
        help="Set timezone for this session only (not saved)",
...
POSIX timezone examples:
  ...
  CET-1CEST,M3.5.0,M10.5.0/3 - Central European with DST""",
    ...
    pcsts = csub.add_parser(
        "set-timezone-save",
        help="Set timezone and save to config (persistent)",
