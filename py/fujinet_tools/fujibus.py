...
def slip_encode(payload: bytes) -> bytes:
    ...
    for b in payload:
        if b == SLIP_END:
            out.append(SLIP_ESCAPE); out.append(SLIP_ESC_END)
        elif b == SLIP_ESCAPE:
            out.append(SLIP_ESCAPE); out.append(SLIP_ESC_ESC)
        else:
            out.append(b)
    out.append(SLIP_END)
    return bytes(out)
...
def send_command(
    ...
    with serial.Serial(port, baud, timeout=timeout) as ser:
        ser.write(packet)
        ser.flush()
        resp = ser.read(read_max)

    if debug:
        print_packet("Incoming raw data", resp)
...
def print_packet(label: str, raw: bytes):
    ...
    # NOTE: pkt is a FujiPacket object now (not a dict)
    print("\n  Header:")
