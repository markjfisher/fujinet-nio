...
class TcpTnfsClient : public ITnfsClient {
private:
...
public:
    explicit TcpTnfsClient(std::unique_ptr<fujinet::io::Channel> channel)
        : _channel(std::move(channel)),
          _sessionId(0),
          _sequenceNum(0)
    {
        FN_LOGI(TAG, "TCP TNFS client created");
    }

    ~TcpTnfsClient() override {
        umount();
    }
...
private:
    bool sendAndReceive(TnfsPacket& packet, std::size_t payloadSize) {
        // Send packet
        std::vector<uint8_t> buffer(PACKET_SIZE);
        std::memcpy(buffer.data(), &packet, 4 + payloadSize);
        _channel->write(buffer.data(), 4 + payloadSize);

        // Wait for response (simple polling approach since Channel doesn't have waitForRead)
        static constexpr int timeoutMs = 2000;
        static constexpr int retryCount = 5;

        for (int i = 0; i < retryCount; ++i) {
            if (_channel->available()) {
                std::size_t bytesRead = _channel->read(reinterpret_cast<uint8_t*>(&packet), sizeof(TnfsPacket));
                if (bytesRead >= 4) {
                    // Check sequence number
                    if (packet.sequenceNum == (_sequenceNum - 1)) {
                        return true;
                    }
                    FN_LOGE(TAG, "Invalid sequence number: expected %d, got %d", _sequenceNum - 1, packet.sequenceNum);
                } else {
                    FN_LOGE(TAG, "Received invalid packet size: %d", static_cast<int>(bytesRead));
                }
            }

            // Simple delay (TODO: implement proper sleep)
            for (int j = 0; j < 100000; ++j) {
                __asm__ __volatile__("nop");
            }
        }

        FN_LOGE(TAG, "Timeout waiting for response");
        return false;
    }
};

std::unique_ptr<ITnfsClient> make_tcp_tnfs_client(std::unique_ptr<fujinet::io::Channel> channel) {
    return std::make_unique<TcpTnfsClient>(std::move(channel));
}
