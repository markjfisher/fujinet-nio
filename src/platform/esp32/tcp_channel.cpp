...
class TcpChannel final : public fujinet::io::Channel {
 public:
  explicit TcpChannel(const std::string& host, uint16_t port) : host_(host), port_(port), socket_fd_(-1), connected_(false) {
    ...
    if (socket_fd_ < 0) {
      FN_LOGE(TAG, "Failed to create TCP socket: %d", errno);
      return;
    }
    ...
    if (flags < 0) {
      FN_LOGE(TAG, "Failed to get socket flags: %d", errno);
      close(socket_fd_);
      socket_fd_ = -1;
      return;
    }
    if (fcntl(socket_fd_, F_SETFL, flags | O_NONBLOCK) < 0) {
      FN_LOGE(TAG, "Failed to set socket to non-blocking: %d", errno);
      close(socket_fd_);
      socket_fd_ = -1;
      return;
    }
    ...
    if (connect(socket_fd_, reinterpret_cast<const sockaddr*>(&server_addr_), sizeof(server_addr_)) < 0) {
      if (errno != EINPROGRESS) {
        FN_LOGE(TAG, "Failed to connect TCP socket: %d", errno);
        close(socket_fd_);
        socket_fd_ = -1;
        return;
      }
      ...
      if (getsockopt(socket_fd_, SOL_SOCKET, SO_ERROR, &sock_error, &len) < 0 || sock_error != 0) {
        FN_LOGE(TAG, "Connection failed: %d", sock_error);
        close(socket_fd_);
        socket_fd_ = -1;
        return;
      }
  ...
  std::size_t read(std::uint8_t* buffer, std::size_t maxLen) override {
    ...
    if (bytes_read < 0) {
      if (errno != EAGAIN && errno != EWOULDBLOCK) {
        FN_LOGE(TAG, "Read failed: %d", errno);
        connected_ = false;
      }
      return 0;
    }
  ...
  void write(const std::uint8_t* buffer, std::size_t len) override {
    ...
    if (bytes_written < 0) {
      FN_LOGE(TAG, "Write failed: %d", errno);
      connected_ = false;
      return;
    }
