...
static std::uint64_t now_ms_monotonic()
{
    // POSIX-only: good enough for connect timeout.
    // If you already have a platform time helper, swap it in.
    struct timespec ts {};
...
void TcpNetworkProtocolPosix::reset_state()
{
    _state = State::Idle;
    _peer_closed = false;
    _read_cursor = 0;
...
void TcpNetworkProtocolPosix::set_error_from_errno(int e)
{
    _last_errno = e;
    _state = State::Error;
}

bool TcpNetworkProtocolPosix::parse_tcp_url(const std::string& url,
                                            std::string& outHost,
...
bool TcpNetworkProtocolPosix::ensure_socket_created()
{
    if (_fd >= 0) return true;

    // Resolve address at connect step (we need host/port first).
    return true;
}

void TcpNetworkProtocolPosix::apply_socket_options()
{
    if (_fd < 0) return;

    if (_opt.nodelay) {
        int v = 1;
        (void)::setsockopt(_fd, IPPROTO_TCP, TCP_NODELAY, &v, sizeof(v));
    }
    if (_opt.keepalive) {
        int v = 1;
        (void)::setsockopt(_fd, SOL_SOCKET, SO_KEEPALIVE, &v, sizeof(v));
    }
...
void TcpNetworkProtocolPosix::step_connect()
{
    if (_state != State::Connecting) return;
    if (_fd < 0) return;

    // connect completion check: poll for write and getsockopt(SO_ERROR)
    struct pollfd pfd {};
    pfd.fd = _fd;
    pfd.events = POLLOUT;
    ...
    if (pr == 0) {
        // timeout check
        const std::uint64_t now = now_ms_monotonic();
...
std::size_t TcpNetworkProtocolPosix::rx_available() const noexcept
{
...
void TcpNetworkProtocolPosix::pump_recv()
{
    if (_fd < 0) return;
...
std::string TcpNetworkProtocolPosix::build_info_headers() const
{
    auto b = std::string{};
    b.reserve(256);
...
fujinet::io::StatusCode TcpNetworkProtocolPosix::open(const fujinet::io::NetworkOpenRequest& req)
{
    ...
    const int gai = ::getaddrinfo(_host.c_str(), portStr.c_str(), &hints, &res);
    if (gai != 0 || !res) {
    ...
    for (auto* ai = res; ai; ai = ai->ai_next) {
        ...
        _connect_start_ms = now_ms_monotonic();
        const int cr = ::connect(_fd, ai->ai_addr, ai->ai_addrlen);
        if (cr == 0) {
            _state = State::Connected;
            lastErr = 0;
            break;
                                                            ...
                                                            std::uint16_t& written)
{
    ...
    const ssize_t n = ::send(_fd, data, len, MSG_DONTWAIT | MSG_NOSIGNAL);
    if (n > 0) {
