...
static constexpr const char* NETSIO_FUJIBUS_TAG = "netsio-fb";
...
public:
    ...
    void write(const std::uint8_t* buffer, std::size_t len) override
    {
        ...
        FN_LOGD(NETSIO_FUJIBUS_TAG, "queued SIO read response: %zu bytes pending=%zu", len, _tx.size());
    }
...
private:
    ...
    {
        if (_tx.empty()) {
            FN_LOGD(NETSIO_FUJIBUS_TAG, "SIO READ requested with no pending response");
            return;
        }
    ...
    {
        if (frame.size() < 4) {
            FN_LOGW(NETSIO_FUJIBUS_TAG, "short SIO command frame: %zu bytes", frame.size());
    ...
    void handle_sync_request(std::uint8_t syncNum)
    {
        ...
        if (_pendingCommand == PendingCommand::Read) {
            ...
            FN_LOGD(NETSIO_FUJIBUS_TAG, "ACK READ sync=%u", static_cast<unsigned>(syncNum));
            _pendingCommand = PendingCommand::None;
    ...
    void handle_data_block(const std::vector<std::uint8_t>& payload)
    {
        ...
        FN_LOGD(NETSIO_FUJIBUS_TAG, "accepted raw NetSIO data block: %zu bytes", payload.size());
    }
    ...
    void handle_data_checksum(std::uint8_t checksum, std::uint8_t syncNum)
    {
        ...
        if (checksum != _pendingWriteChecksum) {
            ...
            FN_LOGD(NETSIO_FUJIBUS_TAG, "SIO write checksum ok: 0x%02X", checksum);
        }
        send_ack_sync(syncNum, SIO_ACK, 0);
        send_sio_complete();
        _pendingCommand = PendingCommand::None;
